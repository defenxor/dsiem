...
# copy ossec.conf from mounted vol
ossec_cfg=/var/ossec/data/etc/ossec.conf
[ ! -e /tmp/firstrun ] && cp -r /root/ossec.conf $ossec_cfg

# first run flag, prevent above recopying upon ossec-control restart
touch /tmp/firstrun

# enable syslog client by default
grep -q "CSYSLOG_DAEMON=ossec-csyslogd" $PLIST 2>/dev/null || echo CSYSLOG_DAEMON=ossec-csyslogd >> $PLIST
